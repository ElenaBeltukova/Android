...
public class Users {
    ...
    public void updateUser(User user){ // Метод редактирования пользователя в БД
        ContentValues values = getContentValues(user);
        database.update(UserDbSchema.UserTable.NAME, values, UserDbSchema.UserTable.Cols.UUID + "=?", new String[]{String.valueOf(user.getUuid())});
    }

    public void deleteUser(User user){ // Метод удаления пользователя в БД
        ContentValues values = getContentValues(user);
        database.delete(UserDbSchema.UserTable.NAME, UserDbSchema.UserTable.Cols.UUID + "=?", new String[]{String.valueOf(user.getUuid())});
    }
